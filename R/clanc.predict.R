...
#' @param clanc.intcv.model a Classification to Nearest Centroids classifier built with \code{clanc.intcv}.
...
"clanc.predict" <- function(clanc.intcv.model, pred.obj, pred.obj.group.id){
  
  gene_names = rownames(pred.obj)
  pred <- predictClanc(pred.obj, gene_names, clanc.intcv.model$model)
  pred_class <- ifelse(pred == "1", "E", "V")
